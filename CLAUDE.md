...
- **CycloneDDS same-host discovery needs the `lo` interface + a localhost peer** (else `ros2` CLI hangs and nodes never see each other): with ONLY the `enx` interface configured, every data socket binds to the enx address and localhost packets are never looped back, so co-host nodes don't discover. `go2_bringup/config/dds/cyclone_dds.xml` lists BOTH `<NetworkInterface name="enx..." multicast="default"/>` (Go2 over the cable) AND `<NetworkInterface name="lo" multicast="false"/>`, plus `<Discovery><Peers><Peer address="localhost"/></Peers>`. Also use the modern `<Interfaces>` form under `<Domain Id="any">` — the deprecated `<NetworkInterfaceAddress>` silently breaks binding. Verified 2026-06-16: round-trip `/cmd_vel`→bridge→`/api/sport/request` works same-host, Go2 still reachable over enx. (`feat/nav2-go2-drive` unblocked.)
