...
- **CycloneDDS `<NetworkInterfaceAddress>` is deprecated and breaks discovery** on the image's cyclonedds: same-host nodes never discover each other and `ros2` CLI hangs. Use the modern form (matches `unitree_ros2/cyclonedds_ws/src/cyclonedds.xml`): `<Interfaces><NetworkInterface name="enx..." priority="default" multicast="default"/></Interfaces>` under `<Domain Id="any"><General>`. NOTE (2026-06-16): even after installing the RMW + modernizing the xml, cyclonedds same-host discovery was still hanging in `glim-humble-docker-jetson` — unresolved; suspect multicast-loopback on the `enx` USB-ethernet or a needed explicit `<Discovery><Peers><Peer address="localhost"/>`. Go2-from-Nav2 driving (`feat/nav2-go2-drive`) is blocked on this.
